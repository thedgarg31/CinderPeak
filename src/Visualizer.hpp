  ...
  public:
    ...
    void visualize_primitives_graph()
    {
      ...
      const std::vector<sf::Color> color_palette = {
          sf::Color(102, 197, 204), 
          sf::Color(246, 207, 113), 
          sf::Color(248, 156, 116), 
          sf::Color(220, 176, 242), 
          sf::Color(135, 197, 125), 
          sf::Color(158, 185, 243), 
      ...
      while (window.isOpen())
      {
        ...
        for (const auto &[src_vertex, edges] : _adj_list)
        {
          ...
          for (const auto &[dest_vertex, edge_weight] : edges)
          {
            ...
            sf::Vector2f control_point1 = src_pos + sf::Vector2f(-unit_direction.y, unit_direction.x) * control_offset;
            sf::Vector2f control_point2 = dest_pos + sf::Vector2f(-unit_direction.y, unit_direction.x) * control_offset;
            ...
            for (int i = 0; i <= segments; ++i)
            {
              ...
              curve.append(sf::Vertex{point, sf::Color(100, 100, 100, 180)}); // Semi-transparent gray
            }
            window.draw(curve);
            ...
            //   sf::Vector2f arrow_left = arrow_head - unit_direction * 10.f + sf::Vector2f(-unit_direction.y, unit_direction.x) * 5.f;
            //   sf::Vector2f arrow_right = arrow_head - unit_direction * 10.f + sf::Vector2f(unit_direction.y, -unit_direction.x) * 5.f;
            ...
            //   arrow[0].color = arrow[1].color = arrow[2].color = sf::Color(100, 100, 100, 180);
            //   window.draw(arrow);
            // }

            sf::Vector2f mid_point = (adjusted_src + adjusted_dest) / 2.f + sf::Vector2f(-unit_direction.y, unit_direction.x) * (control_offset * 0.7f);
            ...
            weight_text.setOrigin(sf::Vector2f{text_bounds.position.x + text_bounds.size.x / 2.f,
                                               text_bounds.position.y + text_bounds.size.y / 2.f});

            weight_text.setPosition(mid_point);

            sf::RectangleShape text_bg(sf::Vector2f(text_bounds.size.x + 8.f, text_bounds.size.y + 4.f));
            // sf::RectangleShape text_bg(sf::Vector2f(text_bounds.size.x + 8.f, text_bounds.size.y + 4.f));
            text_bg.setFillColor(sf::Color(240, 240, 240, 220)); 
            text_bg.setOrigin(sf::Vector2f(text_bg.getSize().x / 2.f, text_bg.getSize().y / 2.f));
        ...
        for (const auto &[vertex, pos] : vertex_positions)
        {
          ...
          highlight.setPosition(sf::Vector2f{pos.x - radius * 0.15f, pos.y - radius * 0.15f});
          ...
          vertex_text.setOrigin(sf::Vector2f{text_bounds.position.x + text_bounds.size.x / 2.f,
                                text_bounds.position.y + text_bounds.size.y / 2.f});
